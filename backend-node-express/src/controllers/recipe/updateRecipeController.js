  ...
  try {
    ...
    for (const { action, components } of compOperations) {
      for (const component of components) {
        ...
        if (action === "update") {
          // Check if recipe_component_id exists in DB and get OLD data of that component_id
          const checkQuery = `
          ...
          oldComponentText = result[0].component_text;
          oldComponentDisplayOrder = result[0].display_order;
        }
    ...
    const ingredientOperations = [
      { action: "add", ingredients: addIngredients || [] },
      { action: "update", ingredients: updateIngredients || [] },
    ];
    for (const { action, ingredients } of ingredientOperations) {
      for (const ing of ingredients) {
        ...
        if (ing.component_display_order !== null && ing.component_display_order !== undefined) {
          if (ing.component_display_order >= maxCompDisplayOrder) {
            return res.status(409).json({
              success: false,
        ...
        // check if new ingredient_id sent in add/update exists in ingredients table or user_ingredients table
        if (ing.ingredient_id !== null && ing.ingredient_id !== undefined) {
          ...
          if (ing.ingredient_source === "main") {
            ...
            const mainIngQuery = `
              ...
              LIMIT 1
            `;
            const mainIngValues = [ing.ingredient_id, user.id];
            ...
            const userIngQuery = `
              ...
              LIMIT 1
            `;
            const userIngValues = [ing.ingredient_id, user.id];
    ...
    try {
      ...
      // Remove steps if any are provided
      for (const step of removeSteps) {
        const removeQuery = `
      ...
      `;
        const removeValues = [recipeId, step.procedure_id];
        const [result] = await conn.query(removeQuery, removeValues);

        // Check if any row was actually updated
        if (result.affectedRows === 0) {
          console.error(`No step found to remove with id ${step.procedure_id}`);
          return res.status(400).json({
            success: false,
            message: "Problem encountered while removing step procedure.",
          });
        }
      }

      // Update/add steps if any are provided
      const stepOperations = [
        { action: "update", steps: data.update_steps || [] },
      ...
      for (const { action, steps } of stepOperations) {
        for (const step of steps) {
          if (action === "update") {
            ...
            const updateQuery = `
              UPDATE recipe_procedures 
              SET step_text = ?, step_order = ?
              WHERE procedure_id = ?
            `;
            const updateValues = [step.step_text, step.step_order, step.procedure_id];
            const [result] = await conn.query(updateQuery, updateValues);
            ...
            if (result.affectedRows === 0) {
              return res.status(400).json({
                success: false,
                message: "Problem encountered while updating steps",
              });
            }
          } else {
            // action === 'add'
          ...
          `;
            ...
            if (inactiveResult.length > 0) {
              procedureId = inactiveResult[0].procedure_id;

              // Update inactive procedure(step) to active
              const updateInactiveQuery = `
                UPDATE recipe_procedures
                SET step_text = ?, 
                step_order = ?, 
                ...
                WHERE procedure_id = ?
              `;
              const updateInactiveValues = [step.step_text, step.step_order, procedureId];
              const updateResult = await conn.query(updateInactiveQuery, updateInactiveValues);
              ...
              if (updateResult.affectedRows === 0) {
                return res.status(400).json({
                  success: false,
                  message: "Problem encountered while adding new step in procedure table",
                });
              }
            } else {
              // Insert new step
              const insertQuery = `
                ...
                VALUES (?, ?, ?)
              `;
              const insertValues = [recipeId, step.step_text, step.step_order];
              const insertResult = await conn.query(insertQuery, insertValues);
